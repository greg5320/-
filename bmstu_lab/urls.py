...
from .views import (
    ...
    MapPoolSubmitView, CompleteOrRejectMapPool, RegisterView, UserLogin, UserUpdate, UserLogout, RemoveMapFromPool,
    UpdateMapPositionView
)

urlpatterns = [
    path('maps/', MapList.as_view(), name='map-list'),
    path('maps/<int:id>/', MapDetail.as_view(), name='map-detail'),
    path('maps/<int:id>/upload-image/', UploadImageForMap.as_view(), name='upload-image'),
    ...
    path('map_pools/<int:map_pool_id>/maps/<int:map_id>/',RemoveMapFromPool.as_view(), name='remove-map-from-map-pool'),
    path('map_pools/<int:map_pool_id>/maps/<int:map_id>/', UpdateMapPositionView.as_view(), name='update_map_position'),
